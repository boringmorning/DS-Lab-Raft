...
func (r *Raft) applyCommand(req *pb.ApplyCommandRequest) (*pb.ApplyCommandResponse, error) {
	...
	return &pb.ApplyCommandResponse{Entry: newLogEntry}, nil
}
...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	if req.LeaderId != r.id && r.state != Follower {
	...
	if len(req.GetEntries()) != 0 {
		...
		for idx, entry := range req.GetEntries() {
			log := r.getLog(entry.Id)
			if log == nil {
				r.appendLogs(req.GetEntries()[idx:])
				break
			} else if log.Term != entry.Term {
				r.deleteLogs(log.Id - 1)
				r.appendLogs(req.GetEntries()[idx:])
				break
			}
		}
	...
	if req.GetLeaderCommitId() > r.commitIndex {
		LastId, _ := r.getLastLog()
		if LastId < req.GetLeaderCommitId() {
			r.setCommitIndex(LastId)
		} else {
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	if (LastTerm > req.GetLastLogTerm()) || (LastTerm == req.GetLastLogTerm() && LastId > req.GetLastLogId()) {
		// TODO: (A.7) - if votedFor is null or candidateId, and candidate’s log is at least as up-to-date as receiver’s log, grant vote
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		if r.nextIndex[peerId] <= LastId {
			nextLog := r.getLog(r.nextIndex[peerId])
			entries = r.getLogs(nextLog.Id)
		}
		...
		go func(peerId uint32, peer Peer, req *pb.AppendEntriesRequest) {
...
func (r *Raft) handleAppendEntriesResult(result *appendEntriesResult) {
	...
	if !result.GetSuccess() {
		...
		r.setNextAndMatchIndex(result.peerId, entries[length-1].GetId()+1, entries[length-1].GetId())
		r.logger.Info("append entries successfully, set next index and match index", zap.Uint32("peer", result.peerId), zap.Uint64("nextIndex", r.nextIndex[result.peerId]), zap.Uint64("matchIndex", entries[length-1].GetId()))
	}
